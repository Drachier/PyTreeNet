...
class TestDMApproachrandomTTN(unittest.TestCase):
    ...
    def test_addition_small_ttn(self):
        ...
        found = dm_addition([ttns1, ttns2, ttns3])
        found = found.to_vector()[0]
        found = found / (found.T.conj()@found)
        npt.assert_array_almost_equal(found, ref)

    def test_some_ttn_no_ttno(self):
        """
        Test the DM approach for some TTNs without a TTNO.
        """
        ttns1 = small_ttns_and_ttno()[0]
        ttns2, ttno2 = small_ttns_and_ttno()
        ttns3, ttno3 = small_ttns_and_ttno()
        ref = deepcopy(ttns1).to_vector()[0]
        ref += deepcopy(ttno2).as_matrix()[0] @ deepcopy(ttns2).to_vector()[0]
        ref += deepcopy(ttno3).as_matrix()[0] @ deepcopy(ttns3).to_vector()[0]
        ref = ref / (ref.T.conj()@ref)
        found = dm_linear_combination([ttns1, ttns2, ttns3],
                                      [None, ttno2, ttno3])
        found = found.to_vector()[0]
    ...
    def test_some_big_ttn_no_ttno(self):
        """
        Test the DM approach for some bigger TTNs without a TTNO.
        """
        ttns1 = big_ttns_and_ttno()[0]
        ttns2, ttno2 = big_ttns_and_ttno()
        ttns3, ttno3 = big_ttns_and_ttno()
...
class TestDMApproachSpecialTTN(unittest.TestCase):
    ...
    def test_some_mps_no_ttno(self):
        """
        Test the DM approach for some MPS without a TTNO.
        """
        ttns1 = random_mps(7, 3, 20, seed=42)
        ttns2, ttno2 = random_mps(7, 3, 20, seed=43), random_mpo(7, 3, 15, seed=25)
        ttns3, ttno3 = random_mps(7, 3, 20, seed=44), random_mpo(7, 3, 15, seed=26)
        ref = deepcopy(ttns1).to_vector()[0]
        ref += deepcopy(ttno2).as_matrix()[0] @ deepcopy(ttns2).to_vector()[0]
        ref += deepcopy(ttno3).as_matrix()[0] @ deepcopy(ttns3).to_vector()[0]
        ref = ref / (ref.T.conj()@ref)
    ...
    def test_binary_some_ttn_no_ttno(self):
        """
        Test the DM approach for a binary operator without a TTNO.
        """
        ttns1 = random_binary_state(3, 2, 10, seed=42)
        ttns2 = random_binary_state(3, 2, 10, seed=43)
        ttno2 = random_binary_operator(3, 2, 7, seed=25)
    ...
    def test_addition_tstar_state(self):
        """
        Test the addition case of the DM approach for a T* TTN state.
        """
        ttns1 = random_tstar_state(3, 2, 15, seed=42)
        ttns2 = random_tstar_state(3, 2, 15, seed=43)
        ttns3 = random_tstar_state(3, 2, 15, seed=44)
    ...
    def test_tstar_some_ttn_no_ttno(self):
        """
        Test the DM approach for a T* state without a TTNO.
        """
        ttns1 = random_tstar_state(3, 2, 15, seed=42)
        ttns2 = random_tstar_state(3, 2, 15, seed=43)
        ttno2 = random_tstar_operator(3, 2, 10, seed=25)
    ...
    def test_ftps_some_ttn_no_ttno(self):
        """
        Test the DM approach for a FTPS without a TTNO.
        """
        ttns1 = random_ftps(3, 2, 10, seed=42)
        ttns2 = random_ftps(3, 2, 10, seed=43)
        ttno2 = random_ftpo(3, 2, 7, seed=25)
