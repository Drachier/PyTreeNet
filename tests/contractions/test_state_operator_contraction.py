...
class TestMatrixElement(TestCase):
    """
    Tests the function get_matrix_element.
    """

    def test_standard(self):
        """
        Test the matrix element of a single node.
        """
        ket, operator = small_ttns_and_ttno()
        bra, _ = small_ttns_and_ttno()
        # Reference
        ket_vec, order = ket.completely_contract_tree(to_copy=True)
        bra_vec, _ = bra.completely_contract_tree(to_copy=True,
                                                  order=order)
        ket_vec = ket_vec.reshape(-1)
        bra_vec = bra_vec.reshape(-1)
        operator_mat, _ = operator.as_matrix(order=order)
        ref = bra_vec @ operator_mat @ ket_vec
        # Found
        found = get_matrix_element(ket, operator, bra)
        npt.assert_allclose(ref, found)
