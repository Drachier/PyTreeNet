...
from pytreenet.util.misc_functions import linear_combination, add, orthogonalise_to, orthogonalise_gram_schmidt, orthogonalise_cholesky
...
class TestMiscFunctionsSmall(unittest.TestCase):
    ...
    def test_add(self):
        ...
        result_ratio = abs(res.scalar_product(self.ttns_1))/abs(res.scalar_product(self.ttns_2))
    ...
    def test_linear_combination(self):
        ...
        c2 = 7.
        # Now perform 1*ttn1+ 2*ttn2
        res = linear_combination([deepcopy(self.ttns_1), deepcopy(self.ttns_2)], [c1,c2], 10)
        res.canonical_form(res.root_id)
        res.normalize()
    ...
    def test_orthogonalise_gram_schmidt(self):
        ...
        ttns_4.normalize()
        res = orthogonalise_gram_schmidt([deepcopy(self.ttns_1), deepcopy(self.ttns_2), deepcopy(self.ttns_3), deepcopy(ttns_4)], 10, 5)
    ...
    def test_orthogonalise_cholesky(self):
        ...
        ttns_4.canonical_form(ttns_4.root_id)
        ttns_4.normalize()
        res = orthogonalise_cholesky([deepcopy(self.ttns_1), deepcopy(self.ttns_2), deepcopy(self.ttns_3), deepcopy(ttns_4)], 10, 5)
...
class TestMiscFunctionsBig(unittest.TestCase):
    ...
    def test_linear_combination(self):
        ...
        c3 = 68.54
        # Now perform 1*ttn1+ 2*ttn2
        res = linear_combination([deepcopy(self.ttns_1), deepcopy(self.ttns_2), deepcopy(self.ttns_3)], [c1,c2,c3], 10)
        res.canonical_form(res.root_id)
        res.normalize()
    ...
    def test_orthogonalise_gram_schmidt(self):
        ...
        ttns_4.canonical_form(ttns_4.root_id)
        ttns_4.normalize()
        state_list = [deepcopy(self.ttns_1), deepcopy(self.ttns_2), deepcopy(self.ttns_3), deepcopy(ttns_4)]
        res = orthogonalise_gram_schmidt(state_list, 8, 10)
        
        for i in range(len(res)):
            self.assertAlmostEqual(res[i].scalar_product(res[i]), 1.)
            for j in range(i):
                self.assertAlmostEqual(res[i].scalar_product(res[j]), 0., places=1)
