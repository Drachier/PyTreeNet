...
6. Contracted TTNDO has correct open legs:
   - For fully intertwined: All nodes have exactly two open legs (representing density matrix indices)
   - For physically intertwined: Physical nodes have two open legs, virtual nodes maintain one open leg
...
9. Expectation values for symmetric and intertwined TTNDOs match for both TTNO and tensor product operators
...
- Symmetric TTNDO: Created from the TTNS using from_ttns_symmetric, which builds a symmetric structure with a central root
- Fully Intertwined TTNDO: Created from the TTNS using from_ttns_fully_intertwined, where all nodes (virtual and physical) have bra/ket pairs
- Physically Intertwined TTNDO: Created using from_ttns_physically_intertwined, which uses dual representation only for physical nodes
"""

import numpy as np
from pytreenet.special_ttn.binary import generate_binary_ttns
from pytreenet.ttns.ttndo import from_ttns_fully_intertwined, from_ttns_symmetric, from_ttns_physically_intertwined, contract_intertwined_ttndo, SymmetricTTNDO
...
def check_contracted_ttndo_bond_dimensions(ttndo, bond_dim):
    """
    Check if the bond dimensions in a contracted intertwined TTNDO match the expected bond dimension.
    
    Args:
        ttndo: The contracted intertwined TTNDO
...
def check_contracted_ttndo_open_legs(ttndo):
    ...
    for node_id, node in nodes.items():
        # For physically intertwined TTNDOs, virtual nodes have one open leg
        if is_physical_form and is_virtual_node(node_id):
            ...
            # Physical nodes (or all nodes in fully intertwined form) should have exactly two open legs
...
def test_network_structure(ttns, ttndo_intertwined, ttndo_symmetric, phys_tensor, num_phys, depth):
    """    
    ...
    2. Symmetric TTNDO (created from TTNS using from_ttns_symmetric):
       - Each node has exactly one open leg
       - Physical tensor values are correctly preserved from input tensor
    ...
    3. Intertwined TTNDO (created from TTNS using from_ttns_fully_intertwined):
    ...
    4. Physically Intertwined TTNDO (created from TTNS using from_ttns_physically_intertwined):
       - Each node has exactly one open leg in original structure
       ...
       - When contracted:
           ...
           - Bond dimensions match expected values
           - The trace equals 1.0 (proper normalization)
           - Both direct trace_contracted_physically_intertwined_ttndo function and class trace() method work correctly
           - TTNO expectation values match symmetric and fully intertwined TTNDOs
           - Tensor product expectation values are consistent across all TTNDO types
    
    Args:
        ttns: The tree tensor network state
        ttndo_intertwined: The intertwined TTNDO structure
    ...
    # Create physically intertwined TTNDO
    print("\nCreating physically intertwined TTNDO...")
    ttndo_physically_intertwined = from_ttns_physically_intertwined(ttns, BOND_DIM, phys_tensor)
    print(f"Physically intertwined TTNDO created with {len(ttndo_physically_intertwined.nodes)} nodes")
    
    # Count nodes in each network
    ttns_nodes = len(ttns.nodes)
    ttndo_intertwined_nodes = len(ttndo_intertwined.nodes)
    ttndo_symmetric_nodes = len(ttndo_symmetric.nodes)
    ttndo_physically_intertwined_nodes = len(ttndo_physically_intertwined.nodes)
    
    print(f"TTNS has {ttns_nodes} nodes")
    print(f"Intertwined TTNDO has {ttndo_intertwined_nodes} nodes")
    print(f"Symmetric TTNDO has {ttndo_symmetric_nodes} nodes")
    print(f"Physically intertwined TTNDO has {ttndo_physically_intertwined_nodes} nodes")
    ...
    try:
        for node in ttndo_intertwined.nodes:
            assert len(ttndo_intertwined.nodes[node].open_legs) == 1, f"Intertwined TTNDO node {node} has {len(ttndo_intertwined.nodes[node].open_legs)} open legs (should be 1)"
        print("✓ All intertwined TTNDO nodes have exactly 1 open leg")
    except AssertionError as e:
        print(f"✗ {e}")
        open_legs_correct = False
    
    try:
        for node in ttndo_physically_intertwined.nodes:
            assert len(ttndo_physically_intertwined.nodes[node].open_legs) == 1, f"Physically intertwined TTNDO node {node} has {len(ttndo_physically_intertwined.nodes[node].open_legs)} open legs (should be 1)"
        print("✓ All physically intertwined TTNDO nodes have exactly 1 open leg")
    except AssertionError as e:
        print(f"✗ {e}")
        open_legs_correct = False
    ...
    try:
        ...
        # Check intertwined TTNDO physical tensors
        print("\nChecking intertwined TTNDO physical tensors...")
        for node_id in ttndo_intertwined.nodes:
            # Check ket physical nodes
            if node_id.startswith('qubit') and node_id.endswith('_ket'):
                tensor = ttndo_intertwined.tensors[node_id]
                
                # The tensor structure is different in intertwined TTNDO:
                ...
                if tensor.ndim == 3 and tensor.shape[2] >= 1:
                    if phys_tensor.ndim == 1:
                        ...
                        for j in range(min(tensor.shape[2], phys_tensor.size)):
                            assert tensor[0, 0, j] == phys_tensor[j], f"Intertwined TTNDO ket node {node_id}: mismatch at [0,0,{j}]: {tensor[0,0,j]} != {phys_tensor[j]}"
                    else:
                        # For 2D input tensor
                        for j in range(min(tensor.shape[2], phys_tensor.shape[1])):
                            assert tensor[0, 0, j] == phys_tensor[0, j], f"Intertwined TTNDO ket node {node_id}: mismatch at [0,0,{j}]: {tensor[0,0,j]} != {phys_tensor[0,j]}"
            ...
            # Check bra physical nodes
                ...
                if tensor.ndim == 2:
                    if phys_tensor.ndim == 1:
                        # For 1D input tensor
                        for j in range(min(tensor.shape[1], phys_tensor.size)):
                            assert tensor[0, j] == np.conj(phys_tensor[j]), f"Intertwined TTNDO bra node {node_id}: mismatch at [0,{j}]: {tensor[0,j]} != {np.conj(phys_tensor[j])}"
                    else:
                        # For 2D input tensor
                        for j in range(min(tensor.shape[1], phys_tensor.shape[1])):
                            assert tensor[0, j] == np.conj(phys_tensor[0, j]), f"Intertwined TTNDO bra node {node_id}: mismatch at [0,{j}]: {tensor[0,j]} != {np.conj(phys_tensor[0,j])}"
        
        print("✓ All intertwined TTNDO physical tensors preserve input values correctly")
    ...
    try:
        # Test intertwined TTNDO virtual nodes
        for node_id in ttndo_intertwined.nodes:
            if is_virtual_node(node_id) and (node_id.endswith("_ket") or node_id.endswith("_bra")):
                tensor = ttndo_intertwined.tensors[node_id]
                assert has_correct_sparse_structure(tensor), f"Intertwined TTNDO virtual node {node_id} doesn't have correct sparse structure"
        print("✓ All intertwined TTNDO virtual nodes have correct sparse structure")
    except AssertionError as e:
        print(f"✗ {e}")
        sparse_structure_correct = False
    ...
    try:
        # Check intertwined TTNDO bond dimensions
        success, error_msg = check_bond_dimensions(ttndo_intertwined, "Intertwined TTNDO")
        assert success, error_msg
        print("✓ All intertwined TTNDO bond dimensions are correct")
    except AssertionError as e:
        print(f"✗ {e}")
        bond_dims_correct = False
    ...
    # 6. Test contracted intertwined TTNDO
    print("\nTesting contracted intertwined TTNDO...")
    contracted_ttndo_correct = True
    
    try:
        # Contract the intertwined TTNDO
        print("Contracting intertwined TTNDO...")
        # For fully intertwined form, all nodes should have two open legs
        ttndo_intertwined_contracted = contract_intertwined_ttndo(ttndo_intertwined)
        print(f"Contracted TTNDO created with {len(ttndo_intertwined_contracted.nodes)} nodes")
        
        # Print information about some nodes
        print("\nSample of contracted TTNDO nodes:")
        sample_nodes = list(ttndo_intertwined_contracted.nodes.keys())[:min(3, len(ttndo_intertwined_contracted.nodes))]
        for node_id in sample_nodes:
            print_node_info(ttndo_intertwined_contracted, node_id, prefix="  ")
        
        # 6a. Check if nodes have exactly two open legs
        print("\nChecking that all nodes have two open legs...")
        success, error_msg = check_contracted_ttndo_open_legs(ttndo_intertwined_contracted)
        assert success, error_msg
        ...
        success, error_msg = check_contracted_ttndo_bond_dimensions(ttndo_intertwined_contracted, BOND_DIM)
        assert success, error_msg
        ...
        trace_value = trace_contracted_fully_intertwined_ttndo(ttndo_intertwined_contracted)
    ...
    # 7. Test expectation values consistency between symmetric and intertwined TTNDOs
    ...
    try:
        ...
        if expectation_values_correct:
            ...
            # For intertwined TTNDO, we need to ensure it's contracted first
            intertwined_contracted = contract_intertwined_ttndo(ttndo_intertwined)
            int_expectation = fully_intertwined_ttndo_ttno_expectation_value(ising_ttno, intertwined_contracted)
            ...
            assert ttno_difference < 1e-10, f"TTNO expectation values differ: symmetric={sym_expectation}, intertwined={int_expectation}, diff={ttno_difference}"
            ...
            for node_id, operator in site_operators.items():
                ...
                # For intertwined TTNDO
                int_tp_expectation = ttndo_intertwined.tensor_product_expectation_value(tensor_product)
                
                # Compare results
                tp_difference = abs(sym_tp_expectation - int_tp_expectation)
                assert tp_difference < 1e-10, f"Tensor product expectation values for {node_id} differ: symmetric={sym_tp_expectation}, intertwined={int_tp_expectation}, diff={tp_difference}"
    ...
    print("\nTesting physically intertwined TTNDO physical tensor preservation...")
    phys_tensor_correct = True
    
    try:
        # Check physical nodes in physically intertwined TTNDO
        for node_id in ttndo_physically_intertwined.nodes:
            # Check ket physical nodes
            if node_id.startswith('qubit') and node_id.endswith('_ket'):
                tensor = ttndo_physically_intertwined.tensors[node_id]
                    ...
                    for i in range(min(tensor.shape[0], original_tensor.shape[0])):
                        for j in range(min(tensor.shape[2], original_tensor.shape[1])):
                            assert np.isclose(tensor[i, 0, j], original_tensor[i, j]), \
                                f"Physically intertwined TTNDO ket node {node_id}: mismatch at [{i},0,{j}]: {tensor[i,0,j]} != {original_tensor[i,j]}"
                ...
                if tensor.ndim == 2:
                    ...
                    for j in range(min(tensor.shape[1], original_tensor.shape[1])):
                        assert np.isclose(tensor[0, j], np.conj(original_tensor[0, j])), \
                            f"Physically intertwined TTNDO bra node {node_id}: mismatch at [0,{j}]: {tensor[0,j]} != {np.conj(original_tensor[0,j])}"
                            
        print("✓ All physically intertwined TTNDO physical tensors preserve values correctly and bra tensors are complex conjugate of ket tensors")
    except AssertionError as e:
        print(f"✗ {e}")
        phys_tensor_correct = False
    
    # 9. Check virtual node sparse structure in physically intertwined TTNDO
    print("\nTesting physically intertwined TTNDO virtual node identity structure...")
    virtual_structure_correct = True
    
    try:
        for node_id in ttndo_physically_intertwined.nodes:
            if is_virtual_node(node_id) and not (node_id.endswith("_ket") or node_id.endswith("_bra")):
                # Virtual nodes in physically intertwined TTNDO don't have bra/ket suffixes
                tensor = ttndo_physically_intertwined.tensors[node_id]
                assert has_correct_sparse_structure(tensor), \
                    f"Physically intertwined TTNDO virtual node {node_id} doesn't have correct sparse structure"
        print("✓ All physically intertwined TTNDO virtual nodes have correct sparse identity structure")
    except AssertionError as e:
        print(f"✗ {e}")
        virtual_structure_correct = False
    
    # 10. Check bond dimensions in physically intertwined TTNDO
    print("\nTesting physically intertwined TTNDO bond dimensions...")
    ...
    try:
        ...
        for node_id in ttndo_physically_intertwined.nodes:
            if is_virtual_node(node_id) and not (node_id.endswith("_ket") or node_id.endswith("_bra")):
                node = ttndo_physically_intertwined.nodes[node_id]
                tensor = ttndo_physically_intertwined.tensors[node_id]
                ...
                for i in range(len(tensor.shape)):
                    if i in node.open_legs:
                        ...
                        assert tensor.shape[i] == 1, \
                            f"Physically intertwined TTNDO virtual node {node_id}: open leg {i} has dimension {tensor.shape[i]} (expected 1)"
                    else:
                        # All other legs should have bond_dim
                        assert tensor.shape[i] == BOND_DIM, \
                            f"Physically intertwined TTNDO virtual node {node_id}: non-open leg {i} has dimension {tensor.shape[i]} (expected {BOND_DIM})"
        ...
        for node_id in ttndo_physically_intertwined.nodes:
            if not is_virtual_node(node_id) and node_id.endswith("_ket"):
                ...
                assert tensor.shape[1] == BOND_DIM, \
                    f"Physically intertwined TTNDO ket physical node {node_id}: lateral leg has dimension {tensor.shape[1]} (expected {BOND_DIM})"
                
                # Physical dimensions should match original
                assert tensor.shape[2] == 2, \
                    f"Physically intertwined TTNDO ket physical node {node_id}: physical leg has dimension {tensor.shape[2]} (expected 2)"
        
        print("✓ All physically intertwined TTNDO bond dimensions are correct")
    except AssertionError as e:
        print(f"✗ {e}")
        physical_bond_dims_correct = False
    
    # 11. Test contracted physically intertwined TTNDO
    print("\nTesting contracted physically intertwined TTNDO...")
    contracted_physical_correct = True
    
    try:
        # Contract the physically intertwined TTNDO
        print("Contracting physically intertwined TTNDO...")
        ttndo_physical_contracted = contract_intertwined_ttndo(ttndo_physically_intertwined)
        print(f"Contracted physically intertwined TTNDO created with {len(ttndo_physical_contracted.nodes)} nodes")
        ...
        assert success, error_msg
        print("✓ All contracted physically intertwined TTNDO nodes have the correct number of open legs")
        
        # 11b. Calculate trace of contracted physically intertwined TTNDO
        print("\nCalculating trace of contracted physically intertwined TTNDO...")
        trace_value = trace_contracted_physically_intertwined_ttndo(ttndo_physical_contracted)
        print(f"Trace result: {trace_value}")
        
        # Check if trace is close to 1.0 (for pure states)
        assert np.isclose(trace_value, 1.0), f"Expected trace 1.0 for pure state, got {trace_value}"
        print("✓ Contracted physically intertwined TTNDO trace is correct (1.0) for pure state")
        
        # 11c. Test the trace method of the IntertwinedTTNDO class directly
        print("\nTesting ttndo_physically_intertwined.trace() method...")
        class_trace_value = ttndo_physically_intertwined.trace()
        ...
        print("✓ IntertwinedTTNDO.trace() method returns correct value (1.0) for physically intertwined TTNDO")
        
    except AssertionError as e:
        print(f"✗ {e}")
        contracted_physical_correct = False
    except Exception as e:
        print(f"✗ Error testing contracted physically intertwined TTNDO: {e}")
        import traceback
        traceback.print_exc()
        contracted_physical_correct = False
    
    # 12. Test expectation values for physically intertwined TTNDO
    print("\nTesting expectation values for physically intertwined TTNDO...")
    physical_expectation_values_correct = True
    

    try:
            # 12a. Generate Ising Hamiltonian and test TTNO expectation values
            print("\nTesting physically intertwined TTNO expectation values with Ising Hamiltonian...")
            ...
            # Calculate expectation value for physically intertwined TTNDO
            physically_contracted = contract_intertwined_ttndo(ttndo_physically_intertwined)
            phys_expectation = physically_intertwined_ttndo_ttno_expectation_value(ising_ttno, physically_contracted)
            
            # Compare with symmetric TTNDO expectation (already calculated)
            phys_ttno_difference = abs(sym_expectation - phys_expectation)
            assert phys_ttno_difference < 1e-10, f"TTNO expectation values differ: symmetric={sym_expectation}, physically intertwined={phys_expectation}, diff={phys_ttno_difference}"
            print(f"✓ Physically intertwined TTNO expectation value matches symmetric TTNDO: {phys_expectation:.10f}")
            
            # Test the direct class method
            class_expectation = ttndo_physically_intertwined.ttno_expectation_value(ising_ttno)
            ...
            print(f"✓ IntertwinedTTNDO.ttno_expectation_value() method returns correct value for physically intertwined TTNDO")
            
            # 12b. Test tensor product expectation values
            print("\nTesting physically intertwined tensor product expectation values...")
            ...
            for node_id in test_sites:
                ...
                # Calculate expectation values
                sym_tp_expectation = ttndo_symmetric.tensor_product_expectation_value(tensor_product)
                phys_tp_expectation = ttndo_physically_intertwined.tensor_product_expectation_value(tensor_product)
                
                # Compare results
                tp_difference = abs(sym_tp_expectation - phys_tp_expectation)
                assert tp_difference < 1e-10, f"Tensor product expectation values for {node_id} differ: symmetric={sym_tp_expectation}, physically intertwined={phys_tp_expectation}, diff={tp_difference}"
        ...
        print(f"✗ Error testing physically intertwined expectation values: {e}")
        import traceback
        traceback.print_exc()
        physical_expectation_values_correct = False
    ...
    for tensor_idx, phys_tensor in enumerate(phys_tensors):
        ...
        for num_phys, depth in test_configs:
            try:
                ...
                # Generate TTNDOs
                ttndo_intertwined = from_ttns_fully_intertwined(ttns, bond_dim=BOND_DIM, phys_tensor=phys_tensor)
                ttndo_symmetric = from_ttns_symmetric(ttns, bond_dim=BOND_DIM)
                
                # Run tests
                result = test_network_structure(ttns, ttndo_intertwined, ttndo_symmetric, phys_tensor, num_phys, depth)
