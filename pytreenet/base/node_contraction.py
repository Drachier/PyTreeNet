...
def operator_expectation_value_on_node(node, operator):
    ...
    # TODO discuss with Richard: this version with less copying

    tensor = node.tensor
    all_axes = list(range(tensor.ndim))
    
    if len(node.open_legs) == 0:
        raise ValueError("A node with no open leg cannot have an operator applied.")
    # TODO: Dimensional checks
    
    node_conjugate_tensor = np.conj(tensor)
    
    # fake node.absorb_tensor
    tensor = np.tensordot(tensor, operator, axes=(node.open_legs[0], 1))
    tensor = np.transpose(tensor, all_axes[:node.open_legs[0]]+[all_axes[-1]]+all_axes[node.open_legs[0]:-1])

    exp_value = complex(np.tensordot(tensor, node_conjugate_tensor,
