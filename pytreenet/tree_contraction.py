...
def check_contracted_identifier(tree_tensor_network, new_identifier):
    if not tree_tensor_network.check_no_node_id_duplication(new_identifier):
...
def contract_nodes_in_tree(
        ttn: TreeTensorNetwork, node1_id: str, node2_id: str, new_tag=None, new_identifier=None):
    """
    ...
    """
    ttn.assert_id_in_tree(node1_id)
    ttn.assert_id_in_tree(node2_id)

    node1 = ttn.nodes[node1_id]
    node2 = ttn.nodes[node2_id]

    (new_tensor_node, new_tensor) = contract_nodes(ttn, node1, node2, new_tag=new_tag, new_identifier=new_identifier)
    ttn.tensors[new_tensor_node.identifier] = new_tensor
    new_identifier = new_tensor_node.identifier
    new_tensor_node.identifier = check_contracted_identifier(ttn, new_identifier=new_identifier)
    ttn.root_id = new_identifier

    # one has to be the parent of the other
    if node1.is_parent_of(node2_id):
        _rewire_neighbours(ttn, node1, node2, new_identifier)
    elif node2.is_parent_of(node1_id):
        _rewire_neighbours(ttn, node2, node1, new_identifier)
    else:
        raise NoConnectionException(f"Nodes with identifiers {node1_id} and {node2_id} are no neigbours.")

    del ttn.nodes[node1_id]
    del ttn.nodes[node2_id]

    ttn.nodes.update({new_tensor_node.identifier: new_tensor_node})
