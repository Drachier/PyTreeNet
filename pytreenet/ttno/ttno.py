...
class TreeTensorNetworkOperator(TreeTensorNetwork):
    ...
    def _create_q_node(tensor, current_node, q_leg_dict):
        """
        Prepares the new TensorNode corresponding to the out tensor.

        Args:
            tensor (np.ndarray): Tensor associated to the new node.
            current_node (Node): The node that is currently in the process of
                being split into two.
            q_leg_dict (Dict[str,int]): Dictionary describing to which node the
                open legs of the tensor in the new node will belong to.
        
        Returns:
            Node: The new tensor node that will replace the current_node.
        """
        q_node = Node(tensor, identifier=current_node.identifier)
