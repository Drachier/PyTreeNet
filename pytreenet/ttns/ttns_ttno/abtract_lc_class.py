...
class AbtractLCwithTempTensors(AbstractLinearCombination):
    """
    An abstract class for linear combinations methods that compute a temporary
    local tensor for each node, that already has the same shape and order as
    is intended for the result TTNS.

    This means for multiple TTNSs, the sum is obtained via direct product of the
    temporary tensors along the new bond, to allow for an increase in bond dimension.
    Such algorithms are the half density matrix algorithm and the successive
    randomised compression algorithm.
    """
    ...
    def __init__(self,
        ...
        """
        super().__init__(ttnss, ttnos, id_trafos_ttns, id_trafos_ttnos)
    ...
    def _node_evaluation(self,
        ...
        for i in range(self.num_ttns()):
            ...
            ket_node_tensor = self.get_ttns_node_tensor(i,
                                                        non_base_node_id)
