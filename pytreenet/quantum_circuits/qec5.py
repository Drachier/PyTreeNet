...
ENCODING_STEPS = 9
def _encoder(circuit: Circuit, num):
    if num == 1:
        # H  H  I  H  I
        circuit.gate("qubit_1", "H", operation_time=1/3)
        circuit.gate("qubit_2", "H", operation_time=1/3)
        circuit.gate("qubit_4", "H", operation_time=1/3)
    elif num == 2:
        # I  C  C  C  Z
        circuit.cu_multi(("qubit_2", "qubit_3", "qubit_4"), "qubit_5", "-I", operation_time=1)
    elif num == 3:
        # I  c  C  c  Z
        circuit.cu_multi(("qubit_2", "qubit_3", "qubit_4"), "qubit_5", "-I", operation_time=1, flip_control={"qubit_2": True, "qubit_3": False, "qubit_4": True})
    elif num == 4:
        # I  I  C  I  X
        circuit.cu("qubit_3", "qubit_5", "X", operation_time=1)
    elif num == 5:
        # C  I  I  I  X
        circuit.cnot("qubit_1", "qubit_5", operation_time=1)
    elif num == 6:
        # C  I  X  I  I
        circuit.cnot("qubit_1", "qubit_3", operation_time=1)
    elif num == 7:
        # I  I  X  C  I
        circuit.cu("qubit_4", "qubit_3", "X", operation_time=1)
    elif num == 8:
        # I  C  I  I  X
        circuit.cu("qubit_2", "qubit_5", "X", operation_time=1)
    elif num == 9:
        # I  I  Z  C  C
        circuit.cu_multi(("qubit_4", "qubit_5"), "qubit_3", "-I", operation_time=1)
    else:
        raise ValueError
    

def repair(circuit, num_bosons=2):
    P0 = np.array([[1, 0], [0, 0]])
    P1 = np.array([[0, 1], [0, 0]])
    ...
    tto = _qubit_tto((myR_1, myR_2, myR_3, myR_4, myR_5), num_bosons)
...
def _qubit_tto(operator_list_pre, num_bosons):
    ...
    for i, t in enumerate(operator_list_pre):
        ...
        t = _add_boson_legs(t, num_connected_qubits, num_bosons)
...
def _add_boson_legs(tensor, start_index, num_bosons):
...
def codeword_tto(circuit, num_bosons=2, reverse=False):
    ...
    t4 = np.array([H])
    t5 = np.array([I])
    tto_list.append(_qubit_tto((t1, t2, t3, t4, t5), num_bosons))
...
def decode(circuit, mode="sim"):
    if mode == "sim":
        for num in range(1, ENCODING_STEPS+1):
            print(f"Decoding {num}/{ENCODING_STEPS}", end="\r")
            _encoder(circuit, ENCODING_STEPS-num+1)
    elif mode == "tto":
        raise NotImplementedError
    else:
        raise ValueError
