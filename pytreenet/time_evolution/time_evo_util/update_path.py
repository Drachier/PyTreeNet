...
class TDVPUpdatePathFinder_LeafToLeaf():
    ...
    def _visit_offpath_subtree_parents(self, branch_origin: str, visited: set) -> List[str]:
        ...
        while (not node.is_root()
            ...
            p_id = node.parent
            # Recursively gather the parent's subtree (children first, then the node)
            path_collected.extend(self._subtree_path_rec(p_id, visited))
            # After handling that parent once, break, 
            # because we've already included climbing further up if needed.
            break
