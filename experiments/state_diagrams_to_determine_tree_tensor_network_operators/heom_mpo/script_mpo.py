...
def run_creation(min_mol, max_mol,
    ...
    for num_bath in tqdm(bath_nums):
        ...
        for num_mol in mol_nums:
            mps = generate_mps_structure(20, 2, 20, 10,
                                        num_mol=num_mol,
            ...
            for term in ham.terms:
                for node_id in term[2].keys():
                    if node_id not in mps.nodes:
                        print(f"Node {node_id} not in MPS")
            ham_pad = ham.pad_with_identities(mps)
            sd = StateDiagram.from_hamiltonian(ham_pad, mps,
                                            method=mode)
            bond_dims = []
...
def save_data(bond_dim_data, metadata):
    save_path = os.getcwd() + "/data/data_mps_" + mode_to_save_str(metadata["mode"]) + "_" + homogenous_to_save_str(metadata["homogenous_mm"]) + ".h5"
