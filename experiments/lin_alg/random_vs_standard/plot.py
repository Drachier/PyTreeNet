"""
Plotting utilities for random vs standard experiments.
"""
import sys
import os
...
def method_name(params: RandomVsStandardParams) -> str:
    ...
    Args:
        params (RandomVsStandardParams): The parameters of the simulation.
...
def extract_rank_vs_err(result: Results
    ...
    """
    Extracts the rank vs. error data from the simulation results.
    ...
    Returns:
        ...
        ranks and corresponding errors.
    """
    rank = result.operator_result(RES_IDS[0])
    errors = result.operator_result(RES_IDS[1])
    return rank, errors

def extract_rank_vs_runtime(result: Results
                         ) -> tuple[NDArray[np.floating], NDArray[np.floating]]:
    """
    Extracts the rank vs. runtime data from the simulation results.
    ...
    Returns:
        tuple[NDArray[np.floating], NDArray[np.floating]]: Two arrays containing
        ranks and corresponding runtimes.
    """
    rank = result.operator_result(RES_IDS[0])
    runtimes = result.operator_result(RES_IDS[2])
    return rank, runtimes
...
def load_data(md_filter: MetadataFilter,
    ...
    params_results = md_filter.load_valid_results_and_parameters(directory_path,
                                                                 parameter_class=RandomVsStandardParams)
    out: dict[str,
