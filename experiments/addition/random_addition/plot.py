"""
Plots the results from the random addition comparison experiments.
"""
import sys
import os
...
def method_name(params: RandomAdditionParams | AdditionMethod) -> str:
    """
    Generates a descriptive name for the addition method used in the simulation.

    Args:
        params (RandomAdditionParams | AdditionMethod): The parameters or method.
    ...
    if isinstance(params, RandomAdditionParams):
...
def method_colour(params: RandomAdditionParams | AdditionMethod) -> str:
    """
...
def num_ttns_linestyle(num_ttns: int) -> str:
    """
    Assigns a linestyle based on the number of TTNS being added.

    Args:
        num_ttns (int): The number of TTNS being added.
    ...
    return linestyles.get(num_ttns, "-")


def build_lineconfig(params: RandomAdditionParams) -> LineConfig:
    """
    Builds a LineConfig object for plotting based on the simulation parameters.

    Args:
        params (RandomAdditionParams): The parameters of the simulation.
...
def load_data(md_filter: MetadataFilter,
    ...
    Returns:
        dict[tuple[str, int], tuple[StandardPlottable, StandardPlottable, StandardPlottable]]:
            A dictionary mapping (method_name, num_ttns) tuples to tuples of plottables
    ...
    params_results = md_filter.load_valid_results_and_parameters(directory_path,
                                                                 parameter_class=RandomAdditionParams)
...
def build_style_mapping(md_filter: MetadataFilter) -> StyleMapping:
    ...
    for method in add_methods:
        style_mapping.set_label("addition_method",
                                method.value,
                                method_name(method))
    style_mapping.add_mapping("num_ttns",
                              StyleOption.LINESTYLE,
                              {num: num_ttns_linestyle(num) for num in md_filter.get_criterium("num_ttns")})
    for num in md_filter.get_criterium("num_ttns"):
        style_mapping.set_label("num_ttns",
                                num,
                                f"N={num}")
...
if __name__ == "__main__":
    ...
    for structure in structures:
        ...
        md_filter.add_to_criterium("num_ttns", [2, 5, 10])

        save_path = os.path.join(plots_dir,
                                 f"random_addition_{structure.value}.pdf")
