...
class TFI_1D_Qutip:
    ...
    def evolve_system_lindblad(self, 
        ...
        evaluation_time : int or "inf", optional
            Difference in time steps between recorded expectation values. Defaults to 1 (every step).
        ...
        """
        # Build times array respecting evaluation_time to skip unnecessary results
        if evaluation_time == "inf":
            # only evaluate at final time
            times = np.array([final_time])
        else:
            # integer evaluation_time controls sampling
            try:
                eval_int = int(evaluation_time)
            except (TypeError, ValueError):
                raise ValueError("evaluation_time must be int or 'inf'")
            if eval_int <= 1:
                # evaluate at every time step
                times = np.arange(0, final_time + time_step_size, time_step_size)
            else:
                # skip intermediate times
                times = np.arange(0, final_time + time_step_size, eval_int * time_step_size)
